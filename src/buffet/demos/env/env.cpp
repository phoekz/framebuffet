...
auto EnvDemo::render(GpuDevice& device, GpuCommandList& cmd) -> void {
    ...
    {
        const auto& compute_pass = _compute;
        const auto& model_pass = _model;
        cmd.set_graphics_constants(ModelBindings {
            .constants = model_pass.constants.cbv_descriptor().index(),
            .vertices = model_pass.vertices.srv_descriptor().index(),
            .lut_texture = compute_pass.lut_texture.srv_descriptor().index(),
            .irr_texture = compute_pass.irr_texture.srv_descriptor().index(),
            .rad_texture = compute_pass.rad_texture.srv_descriptor().index(),
        });
        cmd.set_pipeline(model_pass.pipeline);
        cmd.set_topology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        cmd.set_index_buffer(model_pass.indices.index_buffer_view());
        cmd.draw_indexed_instanced(model_pass.indices.element_size(), 1, 0, 0, 0);
    }
