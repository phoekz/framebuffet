...
void acc_cs(FbComputeInput input) {
    ...
    // Direction.
    const float3 dir = fb_cube_direction_from_dispatch_input(src_id, face_id, dst_texture_size);
    ...
    for (uint i = 0; i < rad_sample_count_per_dispatch; i++) {
        ...
        const float2 u = fb_hammersley2d(global_id, rad_sample_count);
        const float3 n = dir;
        float3 ray_dir;
        float3 weight;
