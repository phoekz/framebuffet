...
class GltfModel {
  public:
    ...
    auto node_transforms() const -> std::span<const Matrix> { return _node_transforms; };
    ...
    auto joint_inverse_binds() const -> std::span<const Matrix> { return _joint_inverse_binds; };
    ...
    auto node_channel_values_t(const GltfChannelHeader& channel) const -> std::span<const Vector3> { return std::span(_node_channel_values_t).subspan(channel.t_offset, channel.t_count); };
    ...
    auto node_channel_values_s(const GltfChannelHeader& channel) const -> std::span<const Vector3> { return std::span(_node_channel_values_s).subspan(channel.s_offset, channel.s_count); };
  ...
  private:
    ...
    std::vector<Matrix> _node_transforms;
    std::vector<uint32_t> _node_parents;
    std::vector<Matrix> _joint_inverse_binds;
    ...
    std::vector<Vector3> _node_channel_values_t;
    std::vector<Quaternion> _node_channel_values_r;
    std::vector<Vector3> _node_channel_values_s;
